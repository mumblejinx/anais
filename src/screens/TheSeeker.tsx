...
export default function TheSeeker() {
  ...
  const runResonanceTest = async () => {
    if (testStage < 2) {
      ...
      toast.success("Check-in complete! +200 XP");
      setActiveTest('none');
      setTestStage(0);
    }
  };
    ...
    >
      <header className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end">
         <div className="md:col-span-2 space-y-4">
            ...
            <p className="text-sm font-mono tracking-widest text-primary uppercase">Places & Activities</p>
         </div>
         <div className="glass-panel p-6 border-l-8 border-secondary flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-bold uppercase opacity-40 mb-2">Activity Score</span>
            {activeTest === 'resonance' ? (
              ...
              >
                ...
                <span className="block text-[10px] mt-1 underline opacity-60 group-hover:opacity-100">Run Check-in</span>
              </button>
            )}
         </div>
      </header>
        ...
        <main className="lg:col-span-8 space-y-12">
           <div className="space-y-6">
              <div className="flex justify-between items-end border-b border-outline-variant pb-4">
                <h4 className="text-2xl font-bold uppercase tracking-tighter text-on-surface">Saved Places</h4>
                <span className="text-[10px] font-bold opacity-30 uppercase tracking-[0.2em]">{anchors.length} places</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {anchors.map(anchor => (
                   ...
                   >
                      <span className="text-[10px] font-bold text-primary uppercase block mb-1">Place</span>
                 ...
                 {anchors.length === 0 && (
                   <div className="col-span-2 py-10 opacity-30 text-xs italic uppercase tracking-widest text-center">No places saved yet.</div>
                 )}
              </div>
           </div>
           ...
           <div className="space-y-6">
              <div className="flex justify-between items-end border-b border-outline-variant pb-4">
                <h4 className="text-2xl font-bold uppercase tracking-tighter text-on-surface">Activity Log</h4>
                <span className="text-[10px] font-bold opacity-30 uppercase tracking-[0.2em]">{expeditions.length} entries</span>
              </div>
              <div className="space-y-4">
                 {expeditions.map(exp => (
                   <motion.div 
                   ...
                   >
                      <div>
                        ...
                        <p className="text-[10px] opacity-40">Notes: {exp.resonance}</p>
                      </div>
                      <div className="w-12 h-1 bg-secondary/20 group-hover:w-24 transition-all duration-500"></div>
                   </motion.div>
                 ))}
                 {expeditions.length === 0 && (
                   <div className="py-10 opacity-30 text-xs italic uppercase tracking-widest text-center">No activities logged yet.</div>
                 )}
              </div>
           </div>
           ...
           <div className="glass-panel p-10 border-2 border-dashed border-tertiary/40 relative overflow-hidden group">
              ...
              <div className="flex flex-col md:flex-row gap-8 items-center relative z-10">
                 ...
                 <div className="space-y-2">
                    <h5 className="text-xl font-bold uppercase tracking-widest text-tertiary">How This Helps</h5>
                    <p className="text-xs opacity-60 leading-relaxed max-w-lg">
                      Places and activities you log here are included as context when ANAIS generates recommendations and responds to your messages in Chat.
                    </p>
                 </div>
              </div>
           </div>
